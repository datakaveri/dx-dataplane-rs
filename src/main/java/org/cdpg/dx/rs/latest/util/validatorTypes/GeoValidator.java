package org.cdpg.dx.rs.latest.util.validatorTypes;
...
public class GeoValidator {
  ...
  private static final List<String> SPATIAL_KEYS =
      List.of(
          NGSILDQUERY_GEOREL,
   ...
   *
   * @param geoQuery JsonObject containing keys "geometry", "geoproperty", "georel", and
   *     "coordinates".
  ...
  private boolean isValidGeoRelation(String geoRel) {
    ...
    if ("near".equalsIgnoreCase(primaryRelation)) {
      if (parts.length != 2) {
        LOGGER.error("Near relation requires exactly one parameter (maxDistance)");
        return false;
      }
      ...
      String[] distanceParts = param.split("=");
      if (distanceParts.length != 2) {
        LOGGER.error("Invalid maxDistance format");
        return false;
      }

      String distanceValue = distanceParts[1].trim();
      return isValidMaxDistance(distanceValue);
    } else {
      if (parts.length > 1) {
        LOGGER.error("Relation {} does not support parameters", primaryRelation);
        return false;
      }
    }
  ...
  private boolean isValidMaxDistance(String distanceStr) {
    try {
      double distance = Double.parseDouble(distanceStr);
      if (distance < MIN_DISTANCE || distance > MAX_DISTANCE) {
        LOGGER.error("maxDistance must be between {} and {}", MIN_DISTANCE, MAX_DISTANCE);
        return false;
      }
      // Check for integer overflow (if stored as integer elsewhere)
      if (distance > Integer.MAX_VALUE) {
        LOGGER.error("maxDistance exceeds maximum integer value");
        return false;
      }
      return true;
    } catch (NumberFormatException ex) {
      LOGGER.error("Invalid maxDistance value: {}", distanceStr);
      return false;
    }
  }
  ...
  private JsonObject buildGeoJson(GeometryType geomType, List<String> coordValues) {
    ...
    switch (geomType) {
      case POINT:
        coordinates.add(parseDouble(coordValues.get(0))).add(parseDouble(coordValues.get(1)));
      ...
      case BBOX:
        ...
        geoJson.put(
            "type", geomType == GeometryType.BBOX ? "LineString" : geomType.getGeoJsonType());
        break;
      case POLYGON:
  ...
  private <T extends Enum<T>> boolean isValidEnum(
      String value, Class<T> enumClass, List<String> allowed) {
  ...
  private boolean isValidField(String value, List<Object> allowed) {
    if (value == null) return true;
