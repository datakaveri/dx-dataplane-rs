...
public class CsvStreamUtil {
    ...
    public static ReadStream<Buffer> toCsvStream(List<ElasticsearchResponse> responses) {
        ...
        if (responses == null || responses.isEmpty()) {
            ...
            responses.forEach(resp -> headers.addAll(resp.getSource().fieldNames()));
        ...
        Buffer csvBuffer = Buffer.buffer(csvBuilder.toString());
        return new ChunkedBufferReadStream(csvBuffer, CHUNK_SIZE);
    }

    public static Buffer toCsvBuffer(List<ElasticsearchResponse> responses, Set<String> headers, boolean writeHeader) {
        StringBuilder csvBuilder = new StringBuilder();
