...
public class OptionalKeyCloakJwtAuthHandler implements AuthenticationHandler {
  private static final Logger LOGGER = LogManager.getLogger(OptionalKeyCloakJwtAuthHandler.class);
  private final JWTAuth jwtAuth;

  public OptionalKeyCloakJwtAuthHandler(JWTAuth jwtAuth) {
    this.jwtAuth = jwtAuth;
  }

  @Override
  public void handle(RoutingContext ctx) {
    LOGGER.info("OptionalKeyCloakJwtAuthHandler invoked");
    ...
    if (token == null || token.isBlank()) {
      ...
      jwtAuth.authenticate(new JsonObject().put("token", token))
              .onComplete(ar -> {
                  if (ar.succeeded()) {
                      LOGGER.debug("Authentication successful for Optional KC JWT");
